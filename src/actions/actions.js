...
import {
    ...
    SET_USER_ID, USER_CONFIRMATION_SUCCESS,
...
export const blogPostListFetch = (page = 1) => {
    return (dispatch) => {
        ...
        return requests.get(`/blog_posts?_page=${page}`).then(response => dispatch(blogPostListReceived(response))).catch(error => dispatch(blogPostListError(error)));
    };
};
...
export const blogPostFetch = (id) => {
    return (dispatch) => {
        ...
        return requests.get(`/blog_posts/${id}`).then(response => dispatch(blogPostReceived(response))).catch(error => dispatch(blogPostError(error)));
    };
};
...
export const commentListFetch = (id, page = 1) => {
    return (dispatch) => {
        ...
        return requests.get(`/blog_posts/${id}/comments?_page=${page}`).then(response => dispatch(commentListReceived(response))).catch(error => dispatch(commentListError(error)));
    };
};
...
export const userLoginAttempt = (username, password) => {
    return (dispatch) => {
        return requests.post('/login_check', {username, password}, false).then(
            response => dispatch(userLoginSuccess(response.token, response.id))).catch(() => {
...
export const userProfileFetch = (userId) => {
    return (dispatch) => {
        ...
        return requests.get(`/users/${userId}`, true).then(response => dispatch(userProfileReceived(userId, response))).catch(() => dispatch(userProfileError(userId)))
    }
};
...
export const userRegister = (username, password, retypedPassword, email, fullname) => {
    return (dispatch) => {
        return requests.post('/users', {
            ...
            email,
            fullname
        }, false).then(() => dispatch(userRegisterSuccess())).catch(error => {
            throw new SubmissionError(
...
export const commentAdd = (comment, blogPostId) => {
    return (dispatch) => {
        ...
        ).then(response => dispatch(commentAdd(response))).catch(error => {
...
export const userConfirm = (confirmationToken) => {
    return (dispatch) => {
        return requests.post('/users/confirm', {confirmationToken}, false).then(() => dispatch(userConfirmationSuccess())).catch(error => {
            throw new SubmissionError({
...
export const imageUpload = (file) => {
    return (dispatch) => {
        ...
        return requests.upload('/images', file).then(response => dispatch(imageUploaded(response))).catch(() => dispatch(imageUploadError))
    }
};
...
export const imageDelete = (id) => {
    return (dispatch) => {
        ...
        return requests.delete(`/images/${id}`).then(() => dispatch(imageDeleted(id)));
    }
};
