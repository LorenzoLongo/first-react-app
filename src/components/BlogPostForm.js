...
class BlogPostForm extends React.Component {
    onSubmit(values) {
        ...
        return blogPostAdd(values.title, values.content, images).then(() => {
            reset();
            history.push('/');
        })
    }
            ...
            <div className="card mt-3 mb-5 shadow-sm">
                <div className="card-body">
                    ...
                    <form onSubmit={handleSubmit(this.onSubmit.bind(this))}>
                        ...
                        <ImageUpload />
                        <ImageBrowser images={images} deleteHandler={imageDelete} isLocked={imageRequestInProgress}/>

                        <button type="submit" className="btn btn-primary btn-block" disabled={submitting || imageRequestInProgress}>Publish Now!</button>
                    </form>
                </div>
            </div>
        )
